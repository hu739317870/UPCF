...
class EvaluatePRinDifferentN(object):
    ...
    def evaluate_PR_in_different_n(self, n=2):
        ...
        while n <= 20:
            ...
            for user in self.test_set:
                count_rs = float(0)
                for movie, prediction_rate in sorted(self.prediction_rate_matrix[user].iteritems(), key=itemgetter(1),
                                                     reverse=True)[:n]:
            ...
            p = p / len(self.test_set)
            r = r / len(self.test_set)
            a = [n, p, r]
            mae_list.append(a)
            print >> sys.stderr, 'n = %s 时的 p = %f, r = %f ' % (n, p, r)
            n += 1
        self.export_file('ml-1m/pr_cosup_dfm.csv', mae_list)
...
if __name__ == '__main__':
    ...
    sim_file = os.path.join('ml-1m', 'prediction_rate_cos_upsim_dfm.csv')
    '''
