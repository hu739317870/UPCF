...
class GenerateCOSUPsim(object):
    ...
    def compute_one_user_COS_UPsim(self, one_user):
        for user in self.user_profile:
            ...
            if user != one_user and int(user) not in range(100, 200):
    ...
    def generate_COS_UPsim(self):
        ...
        print >> sys.stderr, '计算 ColdStart_COS_UPsim 成功'

        COS_UPsim_matrix = DivideData.transform_data_structure(self.COS_UPsim)
        DivideData.export_file('ml-1m/ColdStart_COS_UPsim.csv', COS_UPsim_matrix)
        print >> sys.stderr, '导出 ColdStart_COS_UPsim 成功'

if __name__ == '__main__':
    user_profile_file = os.path.join('ml-1m', 'coldstart_user_profile.csv')
