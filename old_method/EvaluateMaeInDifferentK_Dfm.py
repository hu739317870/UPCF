...
class EvaluateMaeInDiffentN(object):
    ...
    def dfs_prediction_one_user_rate(self, target_user, target_item, k=20):
        ...
        while k <= 200:
            ...
            for user in self.test_set:
                for movie in self.test_set[user]:
                    prediction_rate = self.average_matrix[user] + self.dfs_prediction_one_user_rate(user, movie, k)
            ...
            a = [k, mae]
            mae_list.append(a)
            print >> sys.stderr, 'n = %d 时的 mae = %f' % (k, mae)
            k += 5
        self.export_file('ml-1m/mae_cos_dfm.csv', mae_list)
...
if __name__ == '__main__':
    ...
    sim_file = os.path.join('ml-1m', 'COS_sim.csv')
    '''
